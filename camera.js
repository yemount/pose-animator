...
function setupGui(cameras) {
	...
	gui
		...
		.onChange(() => parseSVG(avatarSvgs[guiState.avatarSVG]));
	multi.open();
...
function detectPoseInRealTime(video) {
	...
	async function poseDetectionFrame() {
		...
		socket.emit('frontPose', { all_poses, faceDetection });
		...
		if (gotPose.length >= 1 && illustration) {
			Skeleton.flipPose(gotPose[0]);

			if (gotFace && gotFace.length > 0) {
				let face = Skeleton.toFaceFrame(gotFace[0]);
				illustration.updateSkeleton(gotPose[0], face);
			} else {
				illustration.updateSkeleton(gotPose[0], null);
			}
...
export async function bindPage() {
	socket.on('backPose', (data) => {
		gotPose = data.all_poses;
		gotFace = data.faceDetection;
	});
	setupCanvas();
